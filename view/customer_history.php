...
if ($stmt_customer === false) {
    die("Lỗi chuẩn bị truy vấn khách hàng: " . $conn->error);
}
...
// Truy vấn lịch sử mua hàng từ bảng order và order_detail, liên kết với bảng product
$sql_history = "SELECT o.id AS order_id, o.order_date, od.quantity, od.unit_price, od.discount, p.name AS product_name
               FROM `order` o
...
if ($stmt_history === false) {
    die("Lỗi chuẩn bị truy vấn lịch sử mua hàng: " . $conn->error);
}
...
<head>
    ...
    <title>Lịch sử mua hàng - <?php echo htmlspecialchars($customer['name']); ?></title>
...
<div id="main">
    ...
    <div class="content">
        <header class="header">
            <h1>Lịch sử mua hàng - <?php echo htmlspecialchars($customer['name']); ?></h1>
            <div class="actions">
        ...
        <div class="card mt-3">
            <div class="card-body">
                ...
                <p><strong>Số điện thoại:</strong> <?php echo htmlspecialchars($customer['phone_number']); ?></p>
                ...
                <table class="table table-bordered">
                    ...
                    <?php if ($result_history->num_rows > 0): ?>
                        <?php while ($row = $result_history->fetch_assoc()): ?>
                            ...
                            $total_price = ($row['unit_price'] * $row['quantity']) - $row['discount'];
                            ?>
                            <tr>
