...
// Lấy thông tin khách hàng để hiển thị trong form
if (!isset($_GET['id'])) {
...
if (!$customer) {
    header("Location: customer.php?error=" . urlencode("Khách hàng không tồn tại."));
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_customer'])) {
    ...
    if (empty($name) || empty($phone_number)) {
        ...
        if ($stmt === false) {
            ...
            if ($stmt->execute()) {
                $stmt->close();
                header("Location: customer.php?customer_updated=success");
                exit();
            } else {
                $error = "Lỗi khi cập nhật khách hàng: " . $stmt->error;
                $stmt->close();
            }
        }
    }
