...
// Kiểm tra biến $order, $customers, $users được truyền từ controller
if (!isset($order) || !isset($customers) || !isset($users)) {
    header("Location: ../view/order.php?error=" . urlencode("Không tìm thấy thông tin đơn hàng."));
    exit();
}
...
<head>
    ...
    <title>Cập nhật đơn hàng - <?php echo htmlspecialchars($shop_name); ?></title>
...
<div id="main">
    ...
    <div class="content">
        <header class="header">
            <h1>Cập nhật đơn hàng - Cơ sở: <?php echo htmlspecialchars($shop_name); ?></h1>
        </header>
        ...
        <div class="card mt-3">
            <div class="card-body">
                <?php if ($order): ?>
                    <form method="POST" action="../controllers/OrderController.php?action=save_update">
                        <input type="hidden" name="id" value="<?php echo htmlspecialchars($order['id']); ?>">
                        <div class="mb-3">
                            <label for="customer_id" class="form-label">Khách hàng <span class="text-danger">*</span></label>
                            <select class="form-control" id="customer_id" name="customer_id" required>
                                <?php foreach ($customers as $customer): ?>
                                    <option value="<?php echo $customer['id']; ?>" <?php echo ($form_data['customer_id'] ?? $order['customer_id']) == $customer['id'] ? 'selected' : ''; ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="employee_id" name="employee_id">
                                ...
                                <?php foreach ($users as $user): ?>
                                    <option value="<?php echo $user['id']; ?>" <?php echo ($form_data['employee_id'] ?? $order['employee_id']) == $user['id'] ? 'selected' : ''; ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" class="form-control" id="order_date" name="order_date" value="<?php echo htmlspecialchars($form_data['order_date'] ?? date('Y-m-d\TH:i', strtotime($order['order_date']))); ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="total_price" class="form-label">Tổng tiền (VNĐ) <span class="text-danger">*</span></label>
                            <input type="number" step="0.01" class="form-control" id="total_price" name="total_price" value="<?php echo htmlspecialchars($form_data['total_price'] ?? $order['total_price']); ?>" required>
                            <small class="form-text text-muted">Hiển thị: <?php echo formatCurrency($form_data['total_price'] ?? $order['total_price']); ?> VNĐ</small>
                        </div>
                        <div class="mb-3">
                            <label for="status" class="form-label">Trạng thái <span class="text-danger">*</span></label>
                            <select class="form-control" id="status" name="status" required>
                                <option value="pending" <?php echo ($form_data['status'] ?? $order['status']) == 'pending' ? 'selected' : ''; ?>>Đang xử lý</option>
                                <option value="completed" <?php echo ($form_data['status'] ?? $order['status']) == 'completed' ? 'selected' : ''; ?>>Hoàn thành</option>
                                <option value="cancelled" <?php echo ($form_data['status'] ?? $order['status']) == 'cancelled' ? 'selected' : ''; ?>>Hủy</option>
                            </select>
                        </div>
